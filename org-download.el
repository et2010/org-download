...
(defun org-download-rename-at-point ()
  ...
  (let* ((dir-path (org-download--dir))
         (str (org-element-property :path (org-element-context)))
         (current-name (file-name-nondirectory (url-unhex-string str)))
         (current-path (concat dir-path current-name))
         (ext (file-name-extension current-name))
         (new-name (concat (read-string "Rename file at point to: " (file-name-sans-extension current-name))) "." ext)
         (new-path (concat dir-path new-name)))
    (rename-file current-path new-path)
    (message "File successfully renamed...")
    (org-download-replace-all (concat "file:" str) new-path)
    (org-display-inline-images)))
